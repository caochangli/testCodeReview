...
// const rollup = require("D:\\Laya2.6.0beta\\LayaAirIDE_beta\\resources\\app\\node_modules\\rollup");
	...
	return rollup.rollup({
		...
		plugins: [
			typescript({
				...
				check: false, //Set to false to avoid doing any diagnostic checks on the code 取消自带的语法监测(会导致编译十几个小时都编译不出来)
			...
			glsl({
				...
				include: /.*(.glsl|.vs|.fs)$/,
				sourceMap: false, //本地使用tsc编译了，这里就取消sourceMap，提升发布时间
				compress:false
			}),
			/*terser({
				output: {
		...
		return bundle.write({
			file: workSpaceDir + '/bin/js/main.min.js',
			format: 'iife',
			name: 'Sgs',
			sourcemap: false
		});
