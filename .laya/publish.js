...
gulp.task("copyLibsJsFile", ["copyFile"], function () {
	...
	//caochangli：在编译结束，未转换压缩游戏代码前，对游戏代码进行处理
	var code = fs.readFileSync(releaseDir + '/js/main.min.js');
	//将编译的es6游戏代码拷贝一份，另存为bundle.js - 用于查询线上问题
	fs.writeFileSync(releaseDir + '/bundle.js', code);
	//将编译的es6游戏代码拷贝一份，另存为main.min.es5.js - 用于转换es5并压缩
	fs.writeFileSync(releaseDir + '/js/main.min.es5.js', code);
...
gulp.task("es6toes5", platformCopyTask, function() {
	if (config.es6toes5) {
		// return gulp.src(`${releaseDir}/js/**/*.js`, { base: releaseDir }) //只对release/web/js目录下进行es6toes5
		return gulp.src(releaseDir + '/js/main.min.es5.js', { base: releaseDir })//只对release/web/js/main.min.es5.js进行es6转es5
		.pipe(babel({
            presets: ['@babel/env'],
            compact: true
		})) 
...
gulp.task("compressJs", ["compressJson"], function () {
	if (config.compressJs) {
		return gulp.src(config.compressJsFilter, { base: releaseDir })
			.pipe(uglify({
				mangle: {
                    keep_fnames:false// 默认 false。传 true来防止压缩器干掉函数名
                }
			}))
