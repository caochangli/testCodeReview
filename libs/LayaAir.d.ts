...
enum IndexFormat {
	...
	class CannonRigidbody3D extends CannonPhysicsCollider  {
		...
		get sleepLinearVelocity():number;
		set sleepLinearVelocity(value:number);
		// get btColliderObject():CANNON.Body;//报错，目前没有用到先注释
...
new Animation_Example();
	...
	class DrawTextureCmd  {
		 ...
		 * @private 
		 */
		static create(texture:Texture,x:number,y:number,width:number,height:number,matrix:Matrix|null,alpha:number,color:string|null,blendMode:string|null,uv?:number[],gray?:boolean):DrawTextureCmd;
...
获取端点列表。
		 ...
		 */
		drawTexture(texture:Texture|null,x?:number,y?:number,width?:number,height?:number,matrix?:Matrix|null,alpha?:number,color?:string|null,blendMode?:string|null,uv?:number[],gray?:boolean):DrawTextureCmd|null;
...
对HTML的显示对象进行排版
	...
	class SoundManager  {
         ...
         * @return SoundChannel对象，通过此对象可以对声音进行控制，以及获取声音信息。
         */
		static playEffect(url:string,loops?:number,complete?:Handler,soundClass?:any, startTime?:number): SoundChannel;
		...
		/**
         * 设置效果音量。根据参数不同，可以分别设置指定声音（背景音乐或音效）音量或者所有音效（不包括背景音乐）音量。
         * @param volume	音量。初始值为1。音量范围从 0（静音）至 1（最大音量）。
         * @param url		(default = null)声音播放地址。默认为null。为空表示设置所有音效（不包括背景音乐）的音量，不为空表示设置指定声音（背景音乐或音效）的音量。
         */
        static setEffectVolume(volume: number, url?: string): void;
	...
	class Loader extends EventDispatcher  {
		 ...
		 * 清理指定资源地址的缓存。
		 * @param url 资源地址。
		 * @param force 是否强制销毁，有些资源是采用引用计数方式销毁，如果force=true，则忽略引用计数，直接销毁，比如Texture，默认为false
		 */
		static clearRes(url:string,force?:boolean):void;
	...
	class LoaderManager extends EventDispatcher  {
		 ...
		 * @param force 是否强制销毁，有些资源是采用引用计数方式销毁，如果force=true，则忽略引用计数，直接销毁，比如Texture，默认为false
		 */
		clearRes(url:string,force?:boolean):void;
...
class VSlider_Example {
	...
	class Browser  {
		 ...
		 * @private 
		 */
		static measureYouText:Function;
...
<b>注意：</b>若 参数 a 不为空，则会改变参数 source 的值为连接后的数组。
         ...
         * @param format 现在的格式 
         */
        static getTextureCompressFormat(format:string):string;
	}
