...
function decoder(mtype) {
    ...
    for (; i < /* initializes */ mtype.fieldsArray.length; ++i) {
        ...
        // var intType = type;    
        // if ((type == "int64" || type == "uint64") && field.name != "userID")
        // {
        //     intType = type + "Number";
        // }
 ...
 * @see {@link Root#load}
 */
function load(filename, root, callback, isLoaded) {
    if (typeof root === "function") {
        callback = root;
        root = new protobuf.Root();
    } else if (!root)
        root = new protobuf.Root();
    return root.load(filename, callback,null,isLoaded);
}
...
Reader._configure = function(BufferReader_) {
    ...
    util.merge(Reader.prototype, {
        ...
        int64Number: function read_int64Number() {
            return readLongVarint.call(this)["toNumber"](false);
        },

        uint64Number: function read_uint64Number() {
 ...
 * @returns {undefined}
 */
Root.prototype.load = function load(filename, options, callback, isLoaded) {
    if (typeof options === "function") {
        callback = options;
        options = undefined;
    }
                ...
                if (parsed.imports)
                    for (; i < parsed.imports.length; ++i)
                        if (resolved = self.resolvePath(filename, parsed.imports[i]))
                            // fetch(resolved);加载引用文件时，过资源管理映射表 caochangli
                            fetch(Laya.ResourceVersion.addVersionPrefix(resolved));
                if (parsed.weakImports)
                    for (i = 0; i < parsed.weakImports.length; ++i)
                        if (resolved = self.resolvePath(filename, parsed.weakImports[i]))
                            // fetch(resolved, true);
                            fetch(Laya.ResourceVersion.addVersionPrefix(resolved), true);
            }
        } catch (err) {
            finish(err);
        }
    ...
    if (isLoaded)//已加载，采用zip方式合并proto
    {
        var file;
        var source;
        var protoName;
        for (var i = 0; i < filename.length; i++)
        {
            ...
            source = file.data;
            protoName = file.name;
            protoName = protoName.substring(0,protoName.length - 4);//去掉压缩打包环境的文件.sgs后缀
            try 
            {
                if (util.isString(source) && source.charAt(0) === "{")
                    source = JSON.parse(source);
                if (!util.isString(source))
                    ...
                    parse.filename = "res/proto/" + protoName + ".proto";//赋值proto文件名，保存于内网环境一至
                    var parsed = parse(source, self, options);
                }
            } catch (err) {
                finish(err);
