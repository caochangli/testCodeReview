	...
	class ScrollBar extends UIComponent {
	    ...
	    changeScrollBar() {
	        ...
	        if (this._showButtons) {
				//caochangli 优化滚动条没有上下按钮还去加载资源，消耗网络请求
				//正常情况下，业务层会提前加载好滚动条资源，如果laya.Loader中没有则表明根本没有这个资源，不需要这里自己再去加载
				var upBtnSkin = this._skin.replace(".png", "$up.png");
				var downBtnSkin = this._skin.replace(".png", "$down.png");
				if (Laya.Loader.getRes(upBtnSkin))
	            	this.upButton.skin = upBtnSkin;
				else
					this.upButton.visible = false;
				if (Laya.Loader.getRes(downBtnSkin))
	            	this.downButton.skin = downBtnSkin;
				else	
