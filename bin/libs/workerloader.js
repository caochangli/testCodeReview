...
onmessage =function (evt){
	...
	{
		if (data.sp3DZip && data.sp3DZipData)//caochangli 将灵zip资源 贴图
		{
			var response, png;
			response = new Uint8Array(data.sp3DZipData);
			doCreateImageBitmap(response,data.url);
			if(!isSet)
			{
				isSet=true;
...
function loadImage2(url)
{
	...
	xhr.onload = function() {
		...
		myTrace("onload:"+url);
		if((xhr.status!=200 && xhr.status!=0) || !response || response.byteLength<10)
		{
			if(!failed)
			{
				failed=true;
