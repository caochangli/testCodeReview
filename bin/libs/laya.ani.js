(function (exports, Laya) {
    ...
    class Skeleton extends Laya.Sprite {
        constructor(templet = null, aniMode = 0) {
            ...
            this.compressTexture = false;//是否纹理压缩
            if (templet)
        ...
        _update(autoKey = true) {
            ...
            //caochangli 上面执行this._player._update(dTime);是会派发STOPPED，业务层收到播放结束事件后可能会回收模板
            if (!this._player || !this._templet || this._templet["_isDestroyed"])
                return;
        ...
        destroy(destroyChild = true) {
            ...
            if (this._player) {//caochangli 动画模板可能不存在(比如：资源还未加载完)
                this._player.offAll();
                this._player = null;
            }
    ...
    class Templet extends AnimationTemplet {
        ...
        _parseTexturePath() {
            ...
            for (i = 0; i < tTextureLen; i++) {
                ...
                if (this._loadList.indexOf(tSrcTexturePath) == -1) {
                    //yanghuipng纹理路径替换成gpu纹理路径
                    if(this.compressTexture)
                    {
                        let oldformat = Laya.ILaya.Utils.getFileExtension(tSrcTexturePath);
                        let format = Laya.ILaya.Utils.getTextureCompressFormat(oldformat);
                        if(format != oldformat)
                        {
                            format = "." + format;
                            oldformat = "." + oldformat;
                            tSrcTexturePath = tSrcTexturePath.replace(oldformat,format);
                        }
                    }
                    //yanghuipng纹理路径替换成gpu纹理路径
        ...
        _parsePublicExtData() {
            ...
            for (i = 0; i < tBoneSlotLen; i++) {
                ...
                //特殊处理，slot名称带特殊字符，则当前插槽开启叠加混色
                if (parent.indexOf("_bl_add") > -1)//兼容老的处理方式
                {
                    tDBBoneSlot.blendType = 1;
                }
                else if (tDBBoneSlot.name)
                {
                    if (tDBBoneSlot.name.indexOf("_bl_add") > -1)//兼容老的处理方式
                    {
                        tDBBoneSlot.blendType = 1;
                    }
                    else 
                    {
                        ...
                        if (blendFlagIndex > -1)//最新处理方式：支持多种混色模式(additive、multiply、screen)
                        {
        ...
        destroy() {
            ...
            for (tTexture in this.subTextureDic) {
                if (tTexture && this.subTextureDic[tTexture]) {//加容错处理
                    this.subTextureDic[tTexture].destroy();
                    delete this.subTextureDic[tTexture];
                }
            }
            for (tTexture in this._textureDic) {
                if (tTexture && this._textureDic[tTexture]) {//加容错处理
