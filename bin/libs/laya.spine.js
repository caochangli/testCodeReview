(function (exports, Laya) {
	        ...
	        for (let i = 0, n = drawOrder.length; i < n; i++) {
	            ...
	            if (texture != null) {
	                ...
	                if (clipper.isClipping()) {
	                    ...
	                    if (this.vertexEffect != null) {
	                        ...
	                        let verts = clippedVertices;
	                        {
	                            for (let v = 0, n = clippedVertices.length; v < n; v += vertexSize) {
									...
									//tonglifang 优化
									mVertices.push(tempPos.x, -tempPos.y);
									if(colors.length < 1){
										colors.push(tempLight.r, tempLight.g, tempLight.b, tempLight.a);
									}
									mUVs.push(tempUv.x, tempUv.y);
	                            }
	                        }
	                    }
	                    else {
						...
						if (this._diySkin && this._diySkin[slot.data.name]) {//caochangli 替换插槽处理 可能有问题
	                    ...
	                    if (this.vertexEffect != null) {
	                    ...
	                    else {
	                        {
	                            for (let v = 2, u = 0, n = renderable.numFloats; v < n; v += vertexSize, u += 2) {
									...
									//tonglifang 优化
									var tempVal1 = uvs[u];
									var tempVal2 = uvs[u + 1];
	                                verts[v + 4] = tempVal1;
	                                verts[v + 5] = tempVal2;
									mVertices.push(verts[v - 2], -verts[v - 1]);
									if(colors.length < 1){
										colors.push(finalColor.r, finalColor.g, finalColor.b, finalColor.a);
									}									
									mUVs.push(tempVal1, tempVal2);
	                            }
	                        }
	                    }
	                    let color = null;
	...
	class SpineSkeleton extends Laya.Sprite {
		...
		setSlotSkin(slotName, texture) {//tonglifang 换插槽接口
			if (!this.skeletonRenderer)
				return;
			if (!this.skeletonRenderer._diySkin){
				this.skeletonRenderer._diySkin = {};
			}
			this.skeletonRenderer._diySkin[slotName] = {texture:texture,uv:{}};
		}
	    stop() {
	        if (!this._pause) {
	            this._pause = true;
	    ...
	    destroy(destroyChild = true) {
			...
			if (this._templet) {//caochangli 动画模板可能不存在(比如：资源还未加载完)
				this._templet._removeReference(1);
	        	this._templet = null;
			}
	        this.timeKeeper = null;
	        this.skeleton = null;
			if (this.state) {//caochangli 动画模板可能不存在(比如：资源还未加载完)
				this.state.clearListeners();
	        	this.state = null;
			}
			if (this.skeletonRenderer) {//替换插槽回收处理
				let diy = this.skeletonRenderer._diySkin;
				if (diy) {
					for (let key in diy)
					{
						let obj = diy[key];
	...
	class SpineTempletBase extends Laya.Resource {
	    ...
	    getAniNameByIndex(index) {
			if (this.isDestroyed || !this.skeletonData || !this.skeletonData.animations || this.skeletonData.animations.length <= index)//caochangli 容错处理
				return null;
	    ...
	    getSkinIndexByName(skinName) {
			if (this.isDestroyed || !this.skeletonData || !this.skeletonData.skins)//caochangli 容错处理
				return -1;
	    ...
	    destroy() {
	        ...
	        for (tTexture in this._textureDic) {
	            ...
	            if (tTexture) {
					...
					//caochangli 回收纹理：this._textureDic中的texture是SpineGLTexture，而Laya.load.loadedMap中存的是Texture2D所以需要调用clearRes
					delete this._textureDic[tTexture];
					Laya.ILaya.loader.clearRes(this._textureDic.root + tTexture);
	            }
	        }
	        super.destroy();
	    }
	...
	class SpineTemplet_4_0 extends SpineTempletBase {
	    ...
	    loop() {
			if (this.isDestroyed) {//caochangli 容错处理
	            this.destroy();
	            return;
	        }
	        if (this.assetManager.isLoadingComplete()) {
	    ...
	    parseSpineAni() {
			...
			this.isParserComplete = true;//tonglifang 解析完成标记
	        this.event(Laya.Event.COMPLETE, this);
	    }
	    buildArmature() {
	        return super.buildArmature();
	    ...
	    destroy() {
			Laya.Laya.timer.clear(this, this.loop);//caochangli
	        super.destroy();
			//tonglifang 回收资源处理
			if(this.atlasUrl)
			{
                Laya.ILaya.loader.clearRes(this.pathPrefix + this.atlasUrl);
                this.atlasUrl = null;//tonglifang确保只清理一次
            }
			if(this.jsonOrSkelUrl)
			{
                Laya.ILaya.loader.clearRes(this.pathPrefix + this.jsonOrSkelUrl);
                this.jsonOrSkelUrl = null;//tonglifang确保只清理一次
            }
	    }
	}
