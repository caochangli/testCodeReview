(function (exports, Laya) {
    ...
    class HTMLStyle {
        ...
        reset() {
            ...
            this.textDecoration = "none";//caochangli null也不等于none 会出现下划线
            this.bgColor = null;
    ...
    HTMLStyle._inheritProps = ["italic", "align", "valign", "leading", "letterSpacing", "stroke", "strokeColor", "bold", "fontWeight", "fontSize", "lineHeight", "wordWrap", "color", "family"];
    ...
    class HTMLImageElement extends HTMLElement {
        ...
        reset() {
            ...
            this._tex = null;
            if (this._url) {//HTMLImageElement加引用计数清理资源 caochangli
                Laya.Browser.window.RES.DelReference(this._url);
                Laya.Browser.window.RES.ClearResByUrl(this._url);
            }
            this._url = null;
            return this;
        }
        set src(url) {
            ...
            this._url = url;
            if (this._url) {//HTMLImageElement加引用计数清理资源 caochangli
                Laya.Browser.window.RES.AddReference(this._url);
            }
        ...
        onloaded() {
            if (!this._style || !this._tex)//加个容错 caochangli
                return;
            var style = this._style;
                ...
                if (xml.nodeType == 3) {
                    ...
                    if (parent instanceof IHtml.HTMLDivParser) {
                        ...
                        if (txt.length > 0) {
                            ...
                            if (node) {
                                // node.style.family = parent.style.family;
                                // node.style.align = parent.style.align;
                        ...
                        if (txt.length > 0) {
                            ...
                            if (parent instanceof HTMLElement && parent.innerTEXT && parent.innerTEXT.length > 0) {
                                ...
                                if (cnode) {
                                    // cnode.style.family = parent.style.family;
                                    // cnode.style.align = parent.style.align;
                    ...
                    if (node) {
                        ...
                        // if(parent && parent.style && node.style){
                        //     node.style.family = parent.style.family;
                        //     node.style.align = parent.style.align;
                        // }
                        node.URI = url;
    ...
    class HTMLDivElement extends Laya.Sprite {
        ...
        _updateGraphic() {
            ...
            // tonglifang 会修改已设置的宽高，应该可以不用调用、get 宽高会去拿element的
